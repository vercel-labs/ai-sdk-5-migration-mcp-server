...
### 🚨 AI AGENTS WILL REFUSE TO PERFORM PHASE 2

**CRITICAL: Phase 2 MUST be performed manually by you (the human developer). AI agents will refuse to execute these migrations.**

Database schema migrations are too critical and risky for AI automation. A single mistake can result in:
- Permanent data loss
- Corrupted database state
- Production downtime
- Loss of user chat histories

**What AI agents CAN do:**
- Help you understand Phase 2 steps
- Answer questions about the migration process
- Review your migration code for potential issues

**What AI agents WILL REFUSE to do:**
- Execute database migrations
- Create migration scripts that modify your database
- Run commands that alter, drop, or rename database tables
- Perform any operations that could cause data loss

### ⚠️ CRITICAL SAFETY REQUIREMENTS FOR MANUAL EXECUTION

**Before you (the human developer) run ANY schema migration:**

1. **Create a complete database backup**
   - Test that you can restore from the backup
   - Store the backup in a secure location separate from your database

2. **Wrap all migration operations in transactions**
   - Ensure automatic rollback on any error
   - Never commit partial migrations

3. **Verify table existence after each step**
   - Check that table renames succeeded
   - Verify new tables were created correctly
   - Ensure no data was lost during operations

4. **Keep the migration script until everything is verified**
   - Don't delete migration code until confirmed working in production
   - Keep a recovery plan documented

5. **Test on a staging/dev database first**
   - NEVER run untested migrations on production
   - Verify the entire migration process works before production deployment
   - Test rollback procedures

**What can go wrong without these safety measures:**
- Table rename operations that delete data instead of renaming
- Migration scripts that fail midway, leaving your database in an inconsistent state
- Loss of production data due to untested migration logic
- No way to recover if something goes wrong

**These safety requirements apply to ALL steps below. You must follow them carefully to prevent data loss.**
